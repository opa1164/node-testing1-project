...
class Seasons {
  ...
  next() {
    // ✨ implement
    this.index++;
    return this.seasons[this.index%4];
  }
}
...
class Car {
  ...
  drive(distance) {
    // ✨ implement
    if(distance !== 0)
    if(this.mpg*this.tank <= distance)
    {
  ...
  refuel(gallons) {
    // ✨ implement
    if(gallons + this.tank <= this.tankSize && gallons > 0)
    {
      this.tank += gallons;
    }
    return this.odometer + this.mpg * this.tank
  }
}
